...
<template>
    ...
    <div class="min-h-screen bg-slate-950 text-white">
        <div class="mx-auto max-w-xl px-4 py-5 space-y-4">
            <div
                v-if="state.player"
            ...
            <section v-if="state.game?.status !== 'lobby' && activeTab === 'turn'" class="rounded-2xl bg-white text-slate-900 px-4 py-4 space-y-3">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs text-slate-500">
                            Turno {{ state.turn?.number || 1 }}/{{ state.game?.total_turns === 0 ? '∞' : state.game?.total_turns }}
                        </p>
                        ...
                        <p class="text-[11px] uppercase text-orange-600 font-semibold">
                            Diff: {{ state.turn?.difficulty ?? state.game?.starting_difficulty }}
                        </p>
                    </div>
                    <span class="text-xs font-semibold text-orange-600">{{ state.turn?.status || state.game?.status }}</span>
                </div>

                <div v-if="state.turn && !state.turn.selected_challenge" class="space-y-2">
                    <article
                    ...
                    >
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-[11px] uppercase text-slate-500">{{ challenge.category || 'Categoria' }}</p>
                                <p class="text-base font-semibold text-slate-900">{{ challenge.title }}</p>
                            </div>
                            <span class="rounded-full bg-slate-900 text-white px-3 py-1 text-[11px] font-semibold">
                                L{{ challenge.level }}
                            </span>
                        </div>
                        <p class="text-sm text-slate-600 mt-1 line-clamp-2">{{ challenge.description || 'Prova sorpresa' }}</p>
                        <p class="text-[11px] text-orange-600 font-semibold mt-1">Max {{ challenge.max_score }} pt</p>
                        <button
                            v-if="state.turn.can_choose && state.turn.status === 'pending'"
                            class="mt-2 w-full rounded-lg bg-slate-900 text-white py-2 text-sm font-semibold disabled:opacity-50"
                            :disabled="loading"
                ...
                <div v-if="state.turn?.selected_challenge" class="rounded-xl border border-orange-200 bg-orange-50 px-3 py-3 space-y-2">
                    <div>
                ...
                <div v-if="state.game?.status === 'finished'" class="rounded-xl bg-slate-900 text-white px-3 py-2 text-sm">
                    Partita conclusa! 🎉
                </div>
            </section>
            ...
            <section v-if="state.game?.status !== 'lobby' && activeTab === 'info'" class="rounded-2xl bg-white/5 px-4 py-4 space-y-3 ring-1 ring-white/10">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[11px] text-white/60">Gestione condivisa.</p>
                    </div>
                </div>
                <div class="space-y-2">
                    <div
